...
for (var i = 0; i < scriptArgs.length; i++) {
  ...
  if (arg.indexOf("-m=") === 0) mode = arg.slice(3).toLowerCase();
    ...
    var raw = arg.slice(3); annotSubtypes = raw.split(",");
    for (var j = 0; j < annotSubtypes.length; j++) { annotSubtypes[j] = annotSubtypes[j].replace(/^\//, "").toLowerCase(); }
  }
...
// -s -i only used by addAnot.js
  else if (arg === "-t") pokeTxt = true;
...
/* ---------------
   hl2txt helper
   returns merged text (non-verbose), or null
----------------*/
function outputHl2txtRow(page, pg, j, annotObj, verbose) {
    print("HL2TXT: enter, pg=" + pg + " j=" + j);

    var sText = page.toStructuredText("preserve-whitespace,preserve-spans");
    print("HL2TXT: sText=" + sText);

    var H = page.getBounds()[3];
    print("HL2TXT: H=" + H);

    var dict = annotObj.getObject();
    print("HL2TXT: dict=" + dict);

    // --- Extract QuadPoints safely ---
    var qp = null;
    try { qp = dict.get("QuadPoints"); } catch (e) { print("HL2TXT: QuadPoints error " + e); }

    if (!qp) {
        var r = dict.get("Rect");
        print("HL2TXT: using Rect, r=" + r);
        if (!r) return null;
        qp = [
            r.get(0), r.get(3),
            r.get(2), r.get(3),
    ...
    var quads = qp;
    print("HL2TXT: quads.length=" + quads.length);
    ...
    for (var q = 0; q < quads.length; q += 8) {
        ...
        print("HL2TXT: quad box " + left + "," + top + " – " + right + "," + bottom);

        var textract = "";
        print("HL2TXT: about to walk structured text");
        sText.walk({
        ...
        print("HL2TXT: textract='" + textract + "'");
    ...
    if (!verbose) {
        ...
        print("HL2TXT: joined='" + joined + "'");
        var line = [ padL(pg+1,4), padL(j+1,4), csvEsc(joined) ].join(',');
        if (!silent) print(line);
        txtOut.write(line + "\n");
        return joined;
    }
...
for (var j = 0; j < annots.length; j++) {
    ...
    if (Debug) print("DBG: annotObj=" + annotObj);
            ...
            var line = verbose
                ? [ padL(pg+1,4), padL(j+1,4), padR(safePDF(author),10), padR(safePDF(subtype),10),
                    padR(safePDF(modified),23), padR(safePDF(rect),44), csvEsc(decoded),
                    csvEsc(info.hex || '') ].join(',')
                : [ padL(pg+1,4), padL(j+1,4), padR(safePDF(subtype),10), padR(safePDF(modified),23),
    ...
    runDeleteMode(
        ...
        function(page) {
            ...
            for (var i = 0; i < ann.length; i++) {
                ...
                if (subtypeMatches(obj, annotSubtypes || [annotSubtype]))
                    out.push(ann[i]);
            }
            return out;
        },
